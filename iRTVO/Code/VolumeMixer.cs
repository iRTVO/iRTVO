...
namespace iRTVO.Code
{
        ...
        private static ISimpleAudioVolume GetVolumeObject(string name)
        {
            ...
            foreach (AudioSession session in AudioUtilities.GetAllSessions())
            {
                if (session.Process != null)
                    logger.Info("{0} , {1}", session.Process.ProcessName,session.Process.MainWindowTitle);
                if ( (session.Process != null) && (session.Process.ProcessName == name) )
                {
                    ISimpleAudioVolume vol = session.GetControl() as ISimpleAudioVolume;
                    return vol;
                }
            }
            return null;
    ...
    {
        internal AudioUtilities.IAudioSessionControl2 _ctl;
        ...
        internal AudioUtilities.IAudioSessionControl2 GetControl()
        {
            return _ctl;
        }
